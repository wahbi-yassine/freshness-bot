...
# =====================================================
ARAB_LEAGUES = [200, 307, 233, 531, 12, 17, 202, 141, 143] # المغرب، السعودية، مصر، البطولة العربية، أبطال أفريقيا/آسيا، الإمارات
EUROPE_LEAGUES = [39, 140, 135, 78, 61, 2, 3] # إنجلترا، إسبانيا، إيطاليا، ألمانيا، فرنسا، الأبطال
NATIONS_LEAGUES = [1, 4, 9, 10, 20, 21, 42] # كأس العالم، اليورو، أمم أفريقيا، وديات، تصفيات

# تنظيف المتغيرات لضمان عدم وجود أسطر زائدة (إصلاح الـ ValueError)
...
def get_wp_headers():
    auth_str = f"{WP_USER}:{WP_APP_PASSWORD.replace(' ', '')}"
    token = base64.b64encode(auth_str.encode()).decode()
    return {"Authorization": f"Basic {token}", "Content-Type": "application/json"}

# =====================================================
# 2. القالب الأحترافي (Widget Style)
# =====================================================
...
(function() {
    function start() {
        ...
        try {
            const jsonStr = decodeURIComponent(escape(atob(payload.innerText.trim())));
            ...
            view.innerHTML = '<div class="ys-no-data">عذراً، حدث خطأ في عرض البيانات.</div>';
        }
    }
...
.ys-tab-btn.active { background: #e60023; color: #fff; border-color: #e60023; }
...
def fetch_data(date_str):
    ...
    try:
        r = requests.get("https://v3.football.api-sports.io/fixtures", headers=headers, params={"date": date_str, "timezone": "Africa/Casablanca"}, timeout=30)
        r.raise_for_status()
...
def update_wp(day_type, data):
    ...
    # تحويل البيانات لـ Base64 لضمان سلامتها من ووردبريس
